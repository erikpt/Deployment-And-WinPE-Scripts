...
# Current status: 
# ------------------------------------------------------------
# HP Drivers: 100% working for download, extract install
# Lenovo Drivers: Experimental
...
function Start-BitsTransferWithRetry {
    ...
    while ($attempt -lt $Retries) {
        try {
            ...
            #Disabled Start-BitsTransfer because it's not available in WinPE without a jumping through a lot of hoops
...
function Get-HPDrivers {
    ...
    foreach ($update in $DriverXmlContent.ImagePal.Solutions.UpdateInfo) {
        ...
        # Extract the driver
        $arguments = "x -o`"$extractFolder`" `"$DriverFilePath`""
        WriteLog "Extracting driver"
        #7z.exe x -oC:\Users\Erik.Pitti\Downloads\drivertest\HP01 ..\sp142682.exe
        try {
            Invoke-Process -FilePath $DriverFilePath -ArgumentList $arguments
        }
        catch {
            WriteLog "Standard extraction failed, using 7-Zip for extraction."
            Invoke-Process -FilePath $7ZipExe -ArgumentList $arguments
            $driverTitle=""
            $driverVendor=""
            ...
            WriteLog "Moved the drivers to their final folder: $FinalDriverFolder"
        }
...
$deviceMake="HP"
$deviceModel="HP EliteBook 650 15.6 inch G9 Notebook PC"
$devicePlatformID="8aa9"
...
try {
    WriteLog "Begin installation of drivers into offline windows image."
	#Add-WindowsDriver -Path "$OSDiskPath" -Driver "$DriversFolder" -Recurse -ErrorAction SilentlyContinue | Out-null
    WriteLog "Driver instatallation completed."
}
catch {
